...
def load_key(path: str) -> bytes:
    """Завантаження ключа з файлу (автоматичне визначення формату: HEX або бінарний)."""
    # Спробуємо спочатку завантажити як бінарний файл
    with open(path, "rb") as f:
        data = f.read()
    
    # Якщо файл має рівно 16 байтів, це бінарний ключ
    if len(data) == 16:
        return data
    
    # Інакше, спробуємо інтерпретувати як HEX
    try:
        ...
        if len(key) != 16:
            raise ValueError(
                f"Ключ у файлі має довжину {len(key)} байтів.\n"
                "Для SM4 потрібен ключ рівно 16 байтів (32 HEX-символи або 16 байтів бінарних даних)."
            )
        return key
    except (UnicodeDecodeError, ValueError) as exc:
        raise ValueError(
            "Не вдалося завантажити ключ.\n"
            "Файл має містити:\n"
            " • 32 HEX-символи (0–9, a–f) в текстовому форматі, АБО\n"
            " • рівно 16 байтів бінарних даних."
        ) from exc


def parse_hex_string(text: str) -> bytes:
    """Парсинг HEX-рядка з дозволеними пробілами/переносами. Повертає байти або піднімає ValueError."""
    cleaned = "".join(text.split()).lower()
    if not cleaned:
        raise ValueError("HEX-рядок порожній.")
    if len(cleaned) % 2 != 0:
        raise ValueError("HEX-рядок має не коректні символи.")
    try:
        return bytes.fromhex(cleaned)
    except ValueError as exc:
        raise ValueError("HEX-рядок містить недопустимі символи. Дозволені 0-9 та a-f (A-F).") from exc


def format_hex_block(data: bytes, bytes_per_line: int = 16) -> str:
    """Форматує байти у читабельний HEX-блок з пробілами і перенесеннями по bytes_per_line."""
    # Кожен елемент bytes ітерується як int, тому форматуємо вручну
